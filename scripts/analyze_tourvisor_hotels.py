    ...
    async with async_playwright() as pw:
        ...
        if clicked:
            ...
            for i in range(40):
                ...
                async def cnt(s):
                    try: return await page.locator(s).count()
                    except: return -1
                res = await cnt(".TVResultItem")
                prog_vis=False
                try: prog_vis = await page.locator(".TVProgressBar").first.is_visible()
                except: pass
                status=""
                try:
                ...
                key=(res,prog_vis); stable=stable+1 if key==last else 0; last=key
                if stable>=4 and res>0: print(f">>> STABILIZED {t}s"); break
        ...
        # Снимок формы
        form = await page.query_selector("body")
