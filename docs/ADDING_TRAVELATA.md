...
**Проверено вживую (headless):** Москва→Египет (2 взр + ребёнок 5, 7-9 ноч, 4-5★, AI) и
СПб→Турция (2 взр, 7-10 ноч) — оба `success`, 10 предложений, отсортированы по цене.

**Ограничения v1 (best-effort/не поддержано):** операторы (в выдаче нет фильтра по ТО),
курорты (можно добавить через сайдбар «Курорты»), режим «Отели» (Travelata = только туры →
`success=False` с понятным текстом). Длительность поиска ~45-90 c (параллельно с Sletat/Tourvisor).
