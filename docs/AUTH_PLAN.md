...
`vip` — права как у `user`, но **поиски без ограничений** (безлимит управляется биллингом,
не матрицей прав); `user` — запуск анализа и своя история, лимит кредитов.

Абстракция прав (permission-строки) сохранена в коде — добавить четвёртую роль = одна
строка в `auth.ROLES` + запись в `ROLE_PERMISSIONS`, без миграции схемы.
