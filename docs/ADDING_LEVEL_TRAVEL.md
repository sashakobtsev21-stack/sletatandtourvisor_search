...
Провайдер `providers/level_travel.py`, `@register_provider("level", experimental=True)`.
Поток: строим deeplink из карт (`_DEPARTURE_SLUG` город→`{City}-{CC}`, `_COUNTRY_CC` страна→ISO-код)
→ `goto` → ждём завершения (карточки стримятся пачками) → **клик «По цене»** → парсим карточки.
- **Список ВИРТУАЛИЗИРОВАН** — в DOM ~5 корневых `[class*=DesktopHotelCard_container]` (не 184;
  «184» в разведке — это под-элементы `[class*=HotelCard]`). Поэтому сортируем **по цене**
  (`button` с текстом «По цене»), чтобы видимые карточки были самыми дешёвыми → честная мин. цена.
- Парсим: `[class*=HotelCardTitle_title]` (имя), `[class*=HotelCardLocation_text]` (курорт),
  `[class*=HotelRating_rating]` (рейтинг 0-10), `[class*=HotelCardPriceBlock_price]` (цена «от N ₽»).
  Звёзды не доверяем (иконки-контуры) → None.
- Сверка `verify_level_search_url` (ночи/взрослые/дети/дата по пути). Город/страна — по картам;
  неизвестные → `success=False` (честно «не поддерживается», чтобы не дать несравнимую выдачу).
- **Проверено headless:** Москва→Турция → cheapest **Cnr İnci 74 938 ₽** (= «от» на сайте). ~40 c.
- **v1 best-effort/не поддержано:** операторы (за «Показать туры»); конкретные курорты (ищем `Any-{CC}`,
  всю страну); число ночей — одиночное `for-{nmin}`; ограниченный список городов вылета (карта).
