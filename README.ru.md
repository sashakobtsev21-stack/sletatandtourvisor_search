...
[English version](README.md) · Русская версия (полная)
...
```
CI включён (`.github/workflows/ci.yml`): ruff, pip-audit (информационный, не гейт), pytest с
покрытием и vitest фронта на каждый push в `main` и на каждый PR. Ночной e2e против живых сайтов
площадок оставлен выключенным намеренно — его вердикт зависит от чужой вёрстки, а не от этого кода
(см. `.github/workflows/README.md`).

Набор правил ruff объявлен явно в `pyproject.toml`, а версия ограничена сверху: дефолтный набор
ruff расширяется от минора к минору, и незакреплённый линтер дал 548 нарушений на неизменившемся
коде. Гейт, вердикт которого зависит от даты прогона, гейтом не является.

**2. Панель «Автотесты» в дашборде** (`/app` → вкладка, только `admin`) — **658 кейсов** по смыслу:
