...
class NotApplicableError(RuntimeError):
    ...
    Контракт: неподходящий режим, направление/город вне карт площадки или запрос за её
    жёсткими лимитами — повтор даст тот же отказ. Оркестратор такой отказ НЕ ретраит,
    UI показывает нейтрально (ℹ️), а не красной ошибкой; текст исключения попадает в
    `ProviderResult.error` чистым, без префикса типа. Где объект исключения доступен,
    классифицируем по isinstance; для строк из БД/старых записей остаётся
    regex-фолбэк `is_not_applicable_error`.
    """


# Площадка вернула success=False, но это НЕ сбой, а «не обслуживает ТАКОЙ запрос»:
# неподходящий режим (Островок в «Турах», Travelata в «Отелях»), направление/город не
# в её картах («недоступен на / недоступна на» — Tourvisor; связка « на » обязательна,
# чтобы не зацепить транзиентное «сервис недоступен» как ложный not-applicable — оно
# должно ретраиться; «не найден в справочнике» — Travelata) или запрос за жёстким
# лимитом площадки («превышает лимит» — окно дат Sletat; именно с «лимит», чтобы НЕ
# зацепить «Превышен таймаут» оркестратора — тот сбой не повторяют по другой причине).
# Такой отказ ДЕТЕРМИНИРОВАН — его показывают нейтрально (а не красной ошибкой) и НЕ
# повторяют (повтор дал бы тот же отказ). Для живых исключений первичен
# isinstance(NotApplicableError); regex — фолбэк для строк из БД/старых записей.
