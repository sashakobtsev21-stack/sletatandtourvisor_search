...
def cap_tokens(store: dict, limit: int = 64) -> None:
    """Ограничить рост словаря токенов: запись /prepare, к которой так и не подключили
    стрим, иначе висела бы вечно. Выкидываем самые старые (dict хранит порядок вставки),
    но НИКОГДА не трогаем сессии с ЖИВОЙ задачей поиска — иначе спам /search/prepare от
    одного клиента выселил бы сессию ИДУЩЕГО поиска другого юзера, и тот терял бы
    реконнект к стриму и кнопку отмены, а поиск-сирота жёг бы слот лимита до конца
    (audit P2). Если живых сессий уже >= limit, словарь временно перерастает кап — это
    близко к реальному пределу одновременных поисков (он защищён отдельным семафором)."""
    if len(store) <= limit:
        return
    for token in list(store):
        if len(store) <= limit:
            break
        task = getattr(store.get(token), "task", None)
        if task is None or task.done():       # выселяем только завершённые/без задачи
